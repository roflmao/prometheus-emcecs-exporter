...
var (
	...
	transactionerrordetail = prometheus.NewDesc(
		prometheus.BuildFQName("emcecs", "cluster", "transaction_error_detail"),
		"error codes broken down by protocol category and error",
		[]string{"vdc", "errorcode", "errorproto", "category"}, nil,
	)
...
func (e *EcsClusterCollector) Collect(ch chan<- prometheus.Metric) {
	...
	// loop over the transaction error types
	for _, element := range fields.TransactionErrors {
